...
class UserapiControllerV4 extends Controller
{
   ...
   public function markascurrentbusiness(Request $request){
        ...
        foreach ($frameList as $key => $value) 
        {
           ...
           $data['frame_url'] = !empty($value->frame_url)?url('/').$value->frame_url:"";
        ...
        if($currntbusiness != null || !empty($currntbusiness) || $currntbusiness != ''){
            ...
            $updatedCurrentBusinessDetails['busi_logo'] = !empty($currntbusiness->busi_logo)?$currntbusiness->busi_logo:"";
...
*/
        ...
        foreach ($adv_datas as $key => $value) 
        {
            ...
            $data['adv_image'] = !empty($value->adv_image)?url('/').$value->adv_image:"";
        ...
        for ($i=0; $i < sizeof($festivals); $i++) { 
            ...
            $data_festival['fest_image'] = !empty($festivals[$i]['fest_image'])?$festivals[$i]['fest_image']:"";
        ...
        for ($i=0; $i < sizeof($incedents); $i++) { 
            ...
            $data_incedent['fest_image'] = !empty($incedents[$i]['fest_image'])?$incedents[$i]['fest_image']:"";
                    ...
                    foreach ($p_plan as $key => $value) 
                    {
                        if ($value->purc_is_cencal == 0 && $value->purc_is_expire == 0) 
                        {
                          /* if ($value->purc_plan_id == 2) 
                           ...
                           $plan_names = Plan::where('status',0)
                                        ->where('plan_id',$value->purc_plan_id)
                                        ->select('plan_name')
                                        ->first();
        ...
        foreach ($category_data as $key => $value) 
        {
            ...
            $data['image'] = !empty($value->image)? url('/').$value->image:"";
        ...
        if(!empty($festivals) || !empty($incedents)){
            return response()->json(['slider' => $advetisement, 'festival' => $festival,'cateogry'=>$incedent, 'business_category'=>$buss_category,'current_business'=>$updatedCurrentBusinessDetails,'premium' => $ispreminum,'current_date' => $currnt_date, 'logout' => $logout,'frameList' => $frameList,'share_message' => $sharemsg,'status' => true,'message'=>'List of all festival']);
        } else {
            return response()->json(['status' => fasle,'message'=>'There is no festival in this month','current_date' => $currnt_date]);
    ...
    public function getMonthsPost(Request $request){
         ...
         $input = $request->all();
            ...
            $posts = Post::where('post_category_id','=',$input['postcategoryid'])->where('post_is_deleted','=',0)->orderBy('image_type','ASC')->orderBy('post_id','DESC')->get();
    ...
    public function getAllVideoPosts(Request $request)
    {
        ...
        foreach ($festivals as $key => $festival) 
        {
            ...
            $data['video_image'] = !empty($festival->video_image)?url('/').$festival->video_image:"";
        ...
        foreach ($incidents as $key => $incident) 
        {
            ...
            $data['video_image'] = !empty($incident->video_image)?url('/').$incident->video_image:"";
    ...
    public function getLanguageVideo(Request $request)
    {
        ...
        {
            $video = DB::table('video_post_data')->where('video_post_id','=',$videoid)->where('is_deleted','=',0)->orderBy('image_type','ASC')->orderBy('id','DESC')->get();
        }
        else
        {
            $video = DB::table('video_post_data')->where('video_post_id','=',$videoid)->whereIn('language_id',$languageid)->where('is_deleted','=',0)->orderBy('image_type','ASC')->orderBy('id','DESC')->get();
        ...
        foreach ($video as $key => $value) 
        {
            ...
            $data['id'] = strval($value->id);
            $data['image'] = !empty($value->thumbnail)?url('/').$value->thumbnail:"";
        ...
        if(count($video) != 0)
        {
            return response()->json(['data' =>$videos,'status' => true,'message'=>"Video List Successfully" ]);
        }
    ...
    public function getLanguagePost(Request $request)
    {
        ...
        if (in_array(0, $languageid)) 
        {
            $post = Post::where('post_category_id','=',$postid)->where('post_is_deleted','=',0)->get();
        }
        else
        {
            $post = Post::where('post_category_id','=',$postid)->whereIn('language_id',$languageid)->where('post_is_deleted','=',0)->get();
        ...
        foreach ($post as $key => $value) 
        {
            ...
            $data['image'] = !empty($value->post_content)?$value->post_content:"";
            $data['type'] = strval($value->image_type);
        ...
        if(count($post) != 0)
        {
            return response()->json(['data' =>$posts,'status' => true,'message'=>"post List Successfully" ]);
        }
    ...
    public function getCustomCategoryPosts(Request $request)
    {
        ...
        foreach ($onlycat as $value) {
            ...
            $temp['custom_image'] = !empty($value->slider_img)?url('/').$value->slider_img:"";
    ...
    public function getLanguageCustomeCategoryPost(Request $request)
    {
        ...
        if (in_array(0, $languageid)) 
        {
            $cat = DB::table('custom_cateogry_data')->where('custom_cateogry_id','=',$catid)->where('is_delete','=',0)->orderBy('image_type','ASC')->orderBy('custom_cateogry_data_id','DESC')->get();
        }
        else
        {
            $cat = DB::table('custom_cateogry_data')->where('custom_cateogry_id','=',$catid)->whereIn('language_id',$languageid)->where('is_delete','=',0)->orderBy('image_type','ASC')->orderBy('custom_cateogry_data_id','DESC')->get();
        ...
        foreach ($cat as $key => $value) 
        {
            ...
            $data['banner_image'] = !empty($value->banner_image)?url('/').$value->banner_image:"";
            $data['image'] = !empty($value->image_one)?url('/').$value->image_one:"";
        ...
        if(count($cat) != 0)
        {
            return response()->json(['data' =>$cats,'status' => true,'message'=>"Custome Category Post List Successfully" ]);
        }
    ...
    public function GetAllFestivalVideo(Request $request)
    {
        ...
        for ($i=0; $i < sizeof($videos); $i++) { 
            ...
            $data_video['video_image'] = !empty($videos[$i]['video_image'])?url('/').$videos[$i]['video_image']:"";
    ...
    public function getBusinessCategoryImages(Request $request)
    {
        ...
        $images = DB::table('business_category_post_data')->where('post_type','image')->where('buss_cat_post_id','=',$request->buss_cat_id)->where('is_deleted','=',0)->get();
        $buss_images = array();
            foreach ($images as $img_key => $img_value) 
            {
                $img_data['image_id'] = strval($img_value->id);
                $img_data['image_url'] = !empty($img_value->thumbnail) ? url('/').$img_value->thumbnail :"";
        ...
        if(!empty($images)){
           ...
           return response()->json(['buss_images' => $buss_images, 'status' => true,'message'=>'List of all images']);
        } else {

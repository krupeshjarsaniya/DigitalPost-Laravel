...
class UserapiControllerV10 extends Controller
{
   ...
   public function markascurrentbusiness(Request $request){
    ...
    public function CurrntbusinessPhoto(Request $request)
    {
        ...
        $isVideoAvailable = false;
        $videoPost = DB::table('business_category_post_data')->where('post_type','video')->where('buss_cat_post_id','=',$request->busi_id)->where('is_deleted','=',0)->where('festival_id', 0)->first();
        if($videoPost) {
            $isVideoAvailable = true;
        }
    ...
    public function SocialLogins(Request $request) {
        ...
        $socialLogin = SocialLogin::where('user_id', $user_id)->where('type', $request->type)->first();
    ...
    public function getLinkedAccounts(Request $request) {
        ...
        foreach($socials as $social) {
            $isLinked = Helper::checkAccountLinked($user_id, $social);
            $accounts->$social = $isLinked;
        }

        return response()->json(['status' => true,'message'=>'Social Account Status', 'data' => $accounts]);
    }
    ...
    public function getScheduledPost(Request $request) {
        ...
        $getPost = DB::table('schedule_post')->where('sp_user_id','=',$user_id)->where('is_posted', 0)->orderBy('sp_id','DESC')->get();
    ...
    public function schedulePost(Request $request) {
        ...
        $social_media_list = explode(',',$_POST['social_media_list']);
        foreach($social_media_list as $social_media) {
            $isLinked = Helper::checkAccountLinked($user_id, $social_media);
            if(!$isLinked) {
                return response()->json(['status'=>false,'message'=> 'Account is not linked for ' . $social_media]);
            }
        }
        ...
        $sp_date_time = Carbon::parse($input['date'] . " " . $sp_time);
    ...
    public function reSchedulePost(Request $request) {
        ...
        $checkPost = DB::table('schedule_post')->where('sp_id', $input['id'])->where('sp_user_id', $user_id)->first();
        if(empty($checkPost)) {
            return response()->json(['status'=>false,'message'=>'post you are looking for is not availabe']);
        }
    ...
    public function sharePost(Request $request) {
        ...
        if(empty($request->social_media_list)){
            return response()->json(['status'=>false,'message'=>'please select social media to publish post']);
        }
        ...
        $social_media_list = explode(',',$request->social_media_list);

        $sp_media_type = $request->media_type;
    ...
    public function getScheduledHistory(Request $request) {
        ...
        $getPost = DB::table('schedule_post')->where('sp_user_id','=',$user_id)->where('is_posted', 1)->orderBy('sp_id','DESC')->skip($offset)->limit($postLimit)->get();
        $posts = array();
        foreach($getPost as &$post){
            $post->sp_media_path = Storage::url($post->sp_media_path);
            $post->sp_social_type = [];
        ...
        $posts_next = DB::table('schedule_post')->where('sp_user_id','=',$user_id)->where('is_posted', 1)->orderBy('sp_id','DESC')->skip($offset + count($getPost))->limit($postLimit)->count();

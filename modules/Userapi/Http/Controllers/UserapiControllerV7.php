...
class UserapiControllerV7 extends Controller
{
   ...
   public function markascurrentbusiness(Request $request){
    ...
    public function updateBusiness(Request $request){
        ...
        if(!is_null($currntbusiness)){
            ...
            if($namechange){
                ...
                $_isPremiumUser = DB::table('purchase_plan')->where('purc_business_id','=', $id)->where('purc_plan_id','!=',3)->first();
    ...
    public function getHomePage(Request $request)
    {
        ...
        if(!empty($frameList)){
            foreach ($frameList as &$frame) {
                $frame->frame_url = !empty($frame->frame_url)?Storage::url($frame->frame_url):"";
            }
        }
        ...
        // $get_data = DB::table('business_category_post_data')->where('post_type','image')->where('is_deleted',0)->orderBy('image_type', 'ASC')->latest('id')->get()->unique(['buss_cat_post_id'])->toArray();
        // $category_data = array_slice($get_data, 0, 10);
        ...
        // {
        //     $category_test = DB::table('business_category')->where('id','=',$value->buss_cat_post_id)->first();
        //     $data = array();    
        //     $data['id'] = strval($category_test->id);
        //     $data['category_name'] = !empty($category_test->name)?$category_test->name:"";
        //     $data['image'] = !empty($category_test->image)? Storage::url($category_test->image):"";
        //     array_push($category,$data);
        //     if(!empty($currntbusiness))
        //     {
        //         if($category_test->name == $currntbusiness->business_category)
        //         {
        //             $keyval = $key;
        //         }
        //     }
        ...
        if(!empty($festivals) || !empty($incedents)){
            return response()->json(['slider' => $advetisement, 'festival' => $festival,'cateogry'=>$incedent, 'business_category'=>$buss_category,'current_business'=>$updatedCurrentBusinessDetails,'premium' => $ispreminum,'current_date' => $currnt_date, 'logout' => $logout,'frameList' => $frameList,'share_message' => $sharemsg ,'currntbusiness_photos' => (object)$currntbusiness_photos,'new_category' => $new_category_dataArray, 'status' => true,'user_language'=>$user_language_check,'message'=>'List of all festival','politicalCurrentBusinessDetails' => $retrunData,'isPoliticalPrimium'=>$politicalCurrentBusinessDetails[1]]);
        } else {
            return response()->json(['status' => fasle,'message'=>'There is no festival in this month','current_date' => $currnt_date]);
    ...
    public function getLanguageVideo(Request $request)
    {
        ...
        if (in_array(0, $languageid)) 
        {
            if($user_language != null )
            {
                ...
                $video_next = DB::table('video_post_data')->where('video_post_id','=',$videoid)->orderBy('image_type','ASC')->orderByRaw("FIELD(language_id , ".$user_language.") DESC")->orderBy('id','DESC')->take($transactionLimit)->skip($offset + count($videos))->get();
            }
            else
            {
                $video_next = DB::table('video_post_data')->where('video_post_id','=',$videoid)->where('is_deleted','=',0)->orderBy('image_type','ASC')->orderBy('id','DESC')->take($transactionLimit)->skip($offset + count($videos))->get();

            }
        }
        else
        {
            $video_next = DB::table('video_post_data')->where('video_post_id','=',$videoid)->whereIn('language_id',$languageid)->where('is_deleted','=',0)->orderBy('image_type','ASC')->orderBy('id','DESC')->take($transactionLimit)->skip($offset + count($videos))->get();
        ...
        {
            return response()->json(['status' => false,'message'=>"Video Not Found", 'meta'=>$meta, 'language'=>$language ]);
    ...
    public function Plans(Request $request)
    {
        ...
        $plans = Plan::where('status','!=',1)->where(function($query) use($plan_type) {
    ...
    function postToTwitter($token, $access_token, $file_path){
        Twitter::reconfig(['consumer_key' => 'sKHLNoxwyaVJnwJKlKo8xmlyk', 'consumer_secret' => 'WUc3kvjkCPqwaMlUm1P67W2SkDbq0R2ThHLChwLD9Xr5TfXBVr', 'token' => '1296046852127764482-6zxKroZJE8D7c1eycC3UQtewwgVhNp', 'secret' => 'o7KEVbXNF1TTzSOWp0gAPhwuRqvNBLQlYzgEJZCNcSvoB']);
    ...
    public function addPoliticalBusiness(Request $request){
        ...
        if($userdata->default_political_business_id == 0 || $userdata->default_political_business_id == ''){
            ...
            $checkBussinesExit = PoliticalBusiness::where('pb_id', $userdata->default_political_business_id)->where('pb_is_deleted', 1)->first();
            if(empty($checkBussinesExit)) {
                User::where('id', $user_id)->update(array(
                    'default_political_business_id' => $business_id,
                ));
            }
    ...
    public function removePoliticalBusiness(Request $request){
        ...
        if($user_data->default_political_business_id == $input['id']) {
            ...
            if(!empty($currntbusiness) || !is_null($currntbusiness)){
                User::where('id', $user_id)->update(array(
                    'default_political_business_id' => $currntbusiness->busi_id,
                ));
            } else {
                User::where('id', $user_id)->update(array(
    ...
    function getPoliticalCurrentBusiness($political_business_id,$user_id){
        $currntbusiness = PoliticalBusiness::where('political_business.pb_id','=',$political_business_id)->leftJoin('purchase_plan','purchase_plan.purc_business_id','=','political_business.pb_id')->leftJoin('plan','plan.plan_id','=','purchase_plan.purc_plan_id')->where('political_business.user_id','=',$user_id)->where('political_business.pb_is_deleted','=',0)->first();
        $updatedCurrentBusinessDetails = array();
    ...
    public function check_api(Request $request)
    {
        ...
        $iPod = stripos($useragent, "android"); 
        return response()->json(['message'=>$useragent]);
    }

...
class UserapiControllerV5 extends Controller
{
   ...
   public function markascurrentbusiness(Request $request){
    ...
    public function getHomePage(Request $request){
        ...
        $incedents = Festival::where('fest_type','=','incident')->where('fest_is_delete','=',0)->orderBy('position_no','ASC')->get();
        ...
        if($currntbusiness != null || !empty($currntbusiness) || $currntbusiness != ''){
            ...
            if(!empty($priminum) || $priminum != null || $priminum != ''){
                $currntbusiness->purc_plan_id = $priminum->purc_plan_id;
                $start_dates = date('Y-m-d');
                if($priminum->purc_plan_id == 1){
                    //$plantrial = Plan::where('plan_sku','=','000FREESKU')->select('plan_validity')->first();
                ...
                $updatedCurrentBusinessDetails['busi_is_approved'] = strval($currntbusiness->busi_is_approved);
                $updatedCurrentBusinessDetails['busi_delete'] = strval($currntbusiness->busi_delete);
                $updatedCurrentBusinessDetails['purc_plan_id'] = strval($currntbusiness->purc_plan_id);
        ...
        if(!empty($festivals) || !empty($incedents)){
            return response()->json(['slider' => $advetisement, 'festival' => $festival,'cateogry'=>$incedent, 'business_category'=>$buss_category,'current_business'=>$updatedCurrentBusinessDetails,'premium' => $ispreminum,'current_date' => $currnt_date, 'logout' => $logout,'frameList' => $frameList,'share_message' => $sharemsg ,'currntbusiness_photos' => (object)$currntbusiness_photos,'new_category' => $new_category_dataArray, 'status' => true,'message'=>'List of all festival']);
        } else {
            return response()->json(['status' => fasle,'message'=>'There is no festival in this month','current_date' => $currnt_date]);
    ...
    public function getMonthsPost(Request $request){
        ...
        if(!empty($posts)){
            return response()->json(['data' => $temp,'status' => true,'message'=>'List of all festival']);
        } else {
    ...
    public function getCustomCategoryPost(){
        ...
        foreach ($onlycat as $value) {
            ...
            foreach ($preference as $value2) {
                ...
                $data['banner_image'] = Storage::url($value2->banner_image);
                $data['custom_cateogry_id'] = $value->custom_cateogry_id;
    ...
    public function getLanguageCustomeCategoryPost(Request $request)
    {
        ...
        foreach ($cat as $key => $value) 
        {
            ...
            $data['banner_image'] = !empty($value->banner_image)?Storage::url($value->banner_image):"";
            $data['image'] = !empty($value->image_one)?Storage::url($value->image_one):"";
    ...
    public function getBusinessCategoryImages(Request $request)
    {
        ...
        $images = DB::table('business_category_post_data')->where('post_type','image')->where('buss_cat_post_id','=',$request->buss_cat_id)->where('is_deleted','=',0)->orderBy('image_type','ASC')->orderBy('id','DESC')->get();
        $buss_images = array();
    ...
    public function CurrntbusinessPhoto(Request $request)
    {
        ...
        $currntbusiness_photo = DB::table('business_category_post_data')->where('post_type','image')->where('buss_cat_post_id','=',$request->busi_id)->where('is_deleted','=',0)->orderBy('image_type','ASC')->orderBy('id','DESC')->get();
        ...
        if(!empty($images)){
           ...
           return response()->json(['images' => $images, 'status' => true,'message'=>'List of all images']);
        } else {
            return response()->json(['status' => false,'message'=>'There is no images']);
